...
#define kPauseImageViewWidth 32.0
...
@interface CXDDXVoiceMeetingDetailViewController ()<RecordingDelegate, PlayingDelegate, UITableViewDelegate, UITableViewDataSource, NSURLSessionDownloadDelegate, UIAlertViewDelegate>
...
/** 结束的语音会议底部背景View */
@property (nonatomic, strong) UIView * endMeetingBottomBackView;
/** 结束的语音会议底部暂停和播放按钮 */
@property (nonatomic, strong) UIButton * endMeetingBottomPlayOrPauseButton;
/** 是否显示上滑取消图片 */
...
- (UILabel *)conferenceTopHeadJobLabel
{
    if(!_conferenceTopHeadJobLabel){
        ...
        _conferenceTopHeadJobLabel.textColor = kConferenceCreateTimeLabelTextColor;
...
- (UISwitch *)conferenceTopAutoPlaySwitch
{
    if(!_conferenceTopAutoPlaySwitch){
        ...
        _conferenceTopAutoPlaySwitch.on = YES;
...
- (UIButton *)bottomRecordButton
{
    if(!_bottomRecordButton){
        ...
        [_bottomRecordButton setTitle:@"按住 发言" forState:UIControlStateNormal];
...
- (UIView *)endMeetingBottomBackView
{
    if(!_endMeetingBottomBackView){
        _endMeetingBottomBackView = [[UIView alloc] init];
        _endMeetingBottomBackView.backgroundColor = RGBACOLOR(244.0, 244.0, 246.0, 1.0);
        _endMeetingBottomBackView.frame = CGRectMake(0, Screen_Height - kBottomRecordBackViewHeight, Screen_Width, kBottomRecordBackViewHeight);
        [self.view addSubview:_endMeetingBottomBackView];
        self.endMeetingBottomPlayOrPauseButton.frame = CGRectMake((Screen_Width - kPauseImageViewWidth)/2, (kBottomRecordBackViewHeight - kPauseImageViewWidth)/2, kPauseImageViewWidth, kPauseImageViewWidth);
    }
    return _endMeetingBottomBackView;
}

- (UIButton *)endMeetingBottomPlayOrPauseButton
{
    if(!_endMeetingBottomPlayOrPauseButton){
        _endMeetingBottomPlayOrPauseButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _endMeetingBottomPlayOrPauseButton.selected = YES;
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateNormal];
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateSelected];
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateHighlighted];
        [_endMeetingBottomPlayOrPauseButton addTarget:self action:@selector(endMeetingBottomPlayOrPauseButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [self.endMeetingBottomBackView addSubview:_endMeetingBottomPlayOrPauseButton];
    }
    return _endMeetingBottomPlayOrPauseButton;
}
...
- (UITableView *)tableView{
    if(!_tableView){
        ...
        _tableView.backgroundColor = RGBACOLOR(235.0, 235.0, 235.0, 1.0);
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"离开会议页面" forKey:@"meetEid"];
...
- (void)popViewController
{
    if(self.type == CXDDXMeetingNotEndType){
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"会议正在进行中，确定离开当前页面?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
- (void)endMeetingBottomPlayOrPauseButtonClick
{
    self.endMeetingBottomPlayOrPauseButton.selected = !self.endMeetingBottomPlayOrPauseButton.selected;
    if(self.endMeetingBottomPlayOrPauseButton.selected){
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateNormal];
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateSelected];
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateHighlighted];
        self.isCancleEnd = NO;
        ...
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"BFIMG"] forState:UIControlStateNormal];
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"BFIMG"] forState:UIControlStateSelected];
        [_endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"BFIMG"] forState:UIControlStateHighlighted];
        [[PlayerManager sharedManager] stopPlaying];
        self.isCancleEnd = YES;
                ...
                [self downloadFileWithCXDDXVoiceModel:sortedArray[i] WithProgress:^(float progress) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if(![self hasHadCXDDXVoiceModel:voiceModel]){
                            ...
                            else if(self.type == CXDDXMeetingNotEndType && !self.conferenceTopAutoPlaySwitch.on){
                                [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:([self.voiceModelArray count] - 1) inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
                            }
                        }
                    });
                }];
...
- (void)tableViewAllCellsRemoveAnimation
{
    dispatch_async(dispatch_get_main_queue(), ^{
        for(NSInteger i = 0;i<[self.voiceModelArray count] ;i++){
            CXDDXVoiceMeetingDetailTableViewCell * cell = (CXDDXVoiceMeetingDetailTableViewCell *)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
            if(cell){
...
- (void)conferenceTopAutoPlaySwitchSwichChanged
{
    ...
    if(self.conferenceTopAutoPlaySwitch.on){
        if(self.type == CXDDXMeetingIsEndType){
            self.endMeetingBottomBackView.hidden = NO;
            ...
            self.isCancleEnd = NO;
            [self loopPlayMeetingMessageFromIndex:([self.voiceModelArray count])];
        }
    }else{
        if(self.type == CXDDXMeetingIsEndType){
            self.endMeetingBottomPlayOrPauseButton.selected = YES;
            [self.endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateNormal];
            [self.endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateSelected];
            [self.endMeetingBottomPlayOrPauseButton setBackgroundImage:[UIImage imageNamed:@"ZTIMG"] forState:UIControlStateHighlighted];
            self.endMeetingBottomBackView.hidden = YES;
    ...
    if(self.isCancleEnd){
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                CXDDXVoiceMeetingDetailTableViewCell * cell = (CXDDXVoiceMeetingDetailTableViewCell *)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
                if(cell){
...
- (void)setVoiceMeetingData{
    ...
    self.conferenceTopHeadJobLabel.text = [NSString stringWithFormat:@"%@ %@",self.model.vedioMeetModel.ygDeptName,self.model.vedioMeetModel.ygJob];
...
//            NSLog(@"播放完成，第%zd条",self.playingIndex - 1);
        ...
        if([[NSUserDefaults standardUserDefaults] objectForKey:PLAYEND_EID_ARRAY] && [[[NSUserDefaults standardUserDefaults] objectForKey:PLAYEND_EID_ARRAY] isKindOfClass:[NSArray class]]){
            playendArray = [[NSMutableArray alloc] initWithArray:[[NSUserDefaults standardUserDefaults] objectForKey:PLAYEND_EID_ARRAY]];
            BOOL hasInsertIntoArray = NO;
            if(playendArray && [playendArray count] > 0){
                for(NSNumber * playendEid in playendArray){
                    if([playendEid integerValue] == [voiceArray[index].eid integerValue]){
            ...
            if(!hasInsertIntoArray){
                [playendArray addObject:voiceArray[index].eid];
            }
        }else{
            playendArray = [[NSMutableArray alloc] initWithCapacity:0];
            [playendArray addObject:voiceArray[index].eid];
        }
        [[NSUserDefaults standardUserDefaults] setValue:playendArray forKey:PLAYEND_EID_ARRAY];
        [[NSUserDefaults standardUserDefaults] synchronize];
    ...
    [HttpTool getWithPath:url params:nil success:^(id JSON) {
        if ([JSON[@"status"] integerValue] == 200) {
            ...
            if([self.model.vedioMeetModel.isEnd integerValue] == 1){
                ...
                [[NSUserDefaults standardUserDefaults] setObject:self.model.vedioMeetModel.eid forKey:@"meetEid"];
                [[NSUserDefaults standardUserDefaults] synchronize];
            }else{
            ...
            [HttpTool postWithPath:url params:params success:^(id JSON) {
                ...
                if ([JSON[@"status"] integerValue] == 200) {
                    ...
                    [self sortVoiceMeetingArray];
                    if(self.type == CXDDXMeetingNotEndType){
                        self.playingIndex = [[self.voiceModelArray lastObject].eid integerValue];
                    }else{
                        self.playingIndex = 0;
                    }
    ...
    if(self.voiceModelArray && [self.voiceModelArray count] > 0){
        ...
        [self downloadFileWithCXDDXVoiceModel:self.voiceModelArray[index] WithProgress:^(float progress) {
            ...
            if([voiceModel.eid integerValue] == [[self.voiceModelArray lastObject].eid integerValue]){
                ...
                self.conferenceTopAutoPlayLabel.frame = CGRectMake(Screen_Width - kConferenceTopAutoPlayLabelRightSpace - kConferenceTopAutoPlayLabelWidth, kHeadViewTopSpace + kConferenceUserNameLabelFontSize + kConferenceUserJobLabelTopSpace + kConferenceUserJobLabelFontSize + kHeadViewTopSpace + kConferenceTopViewMiddleLineHeight + kConferenceTitleLabelTopSpace + kConferenceTitleLabelFontSize + kConferenceAutoPlayLabelTopSpace, kConferenceTopAutoPlayLabelWidth, kConferenceTopAutoPlayLabelFontSize);
                self.conferenceTopAutoPlaySwitch.frame = CGRectMake(Screen_Width - kConferenceAutoPlayBtnRightSpace, kHeadViewTopSpace + kConferenceUserNameLabelFontSize + kConferenceUserJobLabelTopSpace + kConferenceUserJobLabelFontSize + kHeadViewTopSpace + kConferenceTopViewMiddleLineHeight + kConferenceTitleLabelTopSpace + kConferenceTitleLabelFontSize + kConferenceAutoPlayLabelTopSpace - kConferenceAutoPlayBtnTopMoveSpace, kConferenceAutoPlayBtnWidth, kConferenceAutoPlayBtnHeight);
                [self conferenceTopAutoPlaySwitchSwichChanged];
                return;
            }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CXDDXVoiceMeetingDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (nil == cell) {
        cell = [[CXDDXVoiceMeetingDetailTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellID];
    }
    [cell setCXDDXVoiceModel:self.voiceModelArray[indexPath.row]];
    return cell;
}
...
//    NSLog(@"%zd播放结束",self.playingIndex);
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(self.type == CXDDXMeetingIsEndType && self.playingIndex == [self.voiceModelArray count] && self.conferenceTopAutoPlaySwitch.on){
            [[UIApplication sharedApplication].keyWindow makeToast:@"会议播放完毕！" duration:2 position:@"center"];
        }
    });
}

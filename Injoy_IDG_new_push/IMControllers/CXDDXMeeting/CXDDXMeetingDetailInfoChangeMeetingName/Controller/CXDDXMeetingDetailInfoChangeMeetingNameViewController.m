...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString*)string{
    if([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0){
        if ([[[UITextInputMode currentInputMode]primaryLanguage] isEqualToString:@"emoji"]) {
            [[UIApplication sharedApplication].keyWindow makeToast:@"不支持输入表情!" duration:2 position:@"center"];
            return NO;
        }
    }
    return YES;
...
- (void)saveGroupName
{
    ...
    if (_textField && _textField.text && [_textField.text length] > 0 && ![_textField.text isEqualToString:self.model.vedioMeetModel.title]) {
        ...
        [params setValue:self.model.vedioMeetModel.ygDeptId forKey:@"ygDeptId"];
        [params setValue:self.model.vedioMeetModel.ygDeptName forKey:@"ygDeptName"];
        [params setValue:self.model.vedioMeetModel.ygJob forKey:@"ygJob"];

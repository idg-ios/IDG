//
//  CXIDGVoiceMeetingCellTableViewCell.m
//  InjoyIDG
//
...
@interface CXIDGVoiceMeetingCellTableViewCell()<PlayingDelegate>
...
@implementation CXIDGVoiceMeetingCellTableViewCell
...
-(void)setMessage:(CXIMMessage *)message{
    ...
    if (self.isBurnAfterReadMessage && !self.isFromSelf) {
        imgName = @"burnAfterReadVoiceImage3";
    }
...
- (void)play:(NSString *)voicePath {
    ...
    if (self.isBurnAfterReadMessage && !self.isFromSelf) {
        ...
        for (int i = 1; i < 4; i++) {
            NSString *imageName = [NSString stringWithFormat:@"burnAfterReadVoiceImage%d",i];
            UIImage *image = [UIImage imageNamed:imageName];
            if(image){
                [animationImages addObject:image];
            }
...
- (void)reloadImageView{
    ...
    [self.containerView addSubview:_voiceImageView];
}

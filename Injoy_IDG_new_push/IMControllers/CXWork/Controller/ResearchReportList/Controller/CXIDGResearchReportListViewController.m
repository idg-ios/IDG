//
//  CXIDGResearchReportListViewController.m
//  InjoyIDG
//
...
#import "CXIDGResearchReportListTableViewCell.h"
#import "CXProjectSearchView.h"
#import "CXIDGResearchReportListModel.h"
#import "CXResearchReportListSearchView.h"
#import "CXHouseProjectListTableViewCell.h"
#import "CXSearchView.h"

#import "MBProgressHUD+CXCategory.h"

@interface CXIDGResearchReportListViewController ()<UITableViewDelegate, UITableViewDataSource>

@property(strong, nonatomic) UITableView *listTableView;
/** 数据源 */
@property(strong, nonatomic) NSMutableArray<CXIDGResearchReportListModel *> *dataSourceArr;
/** 页码 */
...
/** bgmc */
@property (nonatomic, strong) NSString * bgmc;
/** zy */
@property (nonatomic, strong) NSString * zy;

@property (nonatomic, strong) NSMutableArray *frameArray;
@end

@implementation CXIDGResearchReportListViewController
...
- (NSMutableArray<CXIDGResearchReportListModel *> *)dataSourceArr {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CXResearchReportListDetailViewController *vc = [[CXResearchReportListDetailViewController alloc] init];
    CXIDGResearchReportListModel *listModel = self.dataSourceArr[indexPath.section];
    vc.researchReportListModel = listModel;
...
- (void)getFrameArray:(NSArray *)addArray{
    ...
    [addArray enumerateObjectsUsingBlock:^(CXIDGResearchReportListModel *model , NSUInteger idx, BOOL *stop){
        CXHouseProjectModelFrame *frame = [[CXHouseProjectModelFrame alloc]init];
        frame.researchModel = model;
...
- (void)searchBtnClick{
    ...
    view.cxSearchType = CXSearchReport;
    CXWeakSelf(self);
    view.researchReportBlock = ^(NSString *bgmc, NSString *zy, NSString *keyword) {
        CXStrongSelf(self)
        if(keyword && [keyword length] > 0){
            self.s_projName = keyword;
        }
        if(bgmc && [bgmc length] > 0){
            self.bgmc = bgmc;
        }
        if(zy && [zy length] > 0){
            self.zy = zy;
...
//    self.searchView = [[CXResearchReportListSearchView alloc] initWithFrame:CGRectMake(0, navHigh, Screen_Width, 51.0 + 5*kTitleSpace + 2*kTextFontSize + 2*kTextFieldHeight + 1.0 + 50)];
//    CXWeakSelf(self)
//    self.searchView.onSearchCallback = ^(NSString *bgmc, NSString *zy, NSString *keyword) {
//        CXStrongSelf(self)
//        if(keyword && [keyword length] > 0){
//            self.s_projName = keyword;
//        }
...
- (void)getListWithPage:(NSInteger)page {
    NSString *url = [NSString stringWithFormat:@"%@project/report/list/%ld", urlPrefix, (long) page];
    ...
    [params setValue:self.s_projName forKey:@"s_keyWord"];
    [params setValue:self.bgmc forKey:@"s_docName"];
    [params setValue:self.zy forKey:@"s_summary"];
...
//        [MBProgressHUD hideHUDInMainQueueForView:self.view];//隐藏loading
        ...
        if ([JSON[@"status"] intValue] == 200) {
            ...
            NSArray<CXIDGResearchReportListModel *> *data = [NSArray yy_modelArrayWithClass:[CXIDGResearchReportListModel class] json:JSON[@"data"]];
            if (page == 1) {
...
- (void)popBtnClick{
    if(self.s_projName || self.bgmc || self.zy){
        self.s_projName = nil;
        self.bgmc = nil;
        self.zy = nil;
        self.pageNumber = 1;

...
/** 预览多个文件 单个数组只传一个就好 */
+ (void)previewFileWithPaths:(NSString *)filePath andFileName:(NSString *)fileName{
    YWFilePreviewView *previewView = [[NSBundle mainBundle] loadNibNamed:@"YWFilePreviewView" owner:nil options:nil].lastObject;
    previewView.titleLabel.text = fileName;
    previewView.titleLabel.textAlignment = NSTextAlignmentCenter;
    previewView.titleLabel.textColor = [UIColor whiteColor];
    previewView.filePathArr = @[filePath];
    previewView.frame = CGRectMake(YWS_W, 0, YWS_W, YW_NAV_Hight);
    [YWKeyWindow addSubview:previewView];
    [UIView animateWithDuration:0.20 animations:^{
        previewView.frame = CGRectMake(0, 0, YWS_W, YW_NAV_Hight);
        previewView.backgroundColor = kColorWithRGB(52, 52, 52);

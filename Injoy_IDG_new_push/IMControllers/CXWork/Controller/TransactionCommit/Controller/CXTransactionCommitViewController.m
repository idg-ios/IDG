//
//  CXTransactionCommitViewController.m
//  InjoyERP
//
...
@interface CXTransactionCommitViewController ()<UITableViewDelegate, UITableViewDataSource,CXWorkCreateDetailViewReloadHeightDelegate>
...
@implementation CXTransactionCommitViewController
...
- (void)setupView
{
    ...
    [self.rootTopView setNavTitle:LocalString(@"事务报告")];
    ...
    _theTableHeaderView.frame = CGRectMake(0, 0, Screen_Width, 3*kCellHeight + self.contentDetailViewHeight + 3);
    ...
    self.annexLabel.frame = CGRectMake(kLeftSpace, kCellHeight + 1 + kCellHeight + self.contentDetailViewHeight + (kCellHeight - kFontSizeValueForForm)/2, self.annexLabel.size.width, kFontSizeValueForForm);
    [_theTableHeaderView addSubview:self.annexLabel];
    
    self.annexView = [[CXOLDERPAnnexView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.annexLabel.frame), kCellHeight + 1 + kCellHeight + self.contentDetailViewHeight, Screen_Width - CGRectGetMaxX(self.annexLabel.frame) - kLeftSpace, kCellHeight) AndViewWidth:Screen_Width - CGRectGetMaxX(self.annexLabel.frame) - kLeftSpace];
    self.annexView.vc = self;
    [_theTableHeaderView addSubview:self.annexView];
    
    self.thirdLineView = [[UIView alloc] init];
    self.thirdLineView.frame = CGRectMake(0, kCellHeight + 1 + kCellHeight + self.contentDetailViewHeight + kCellHeight + 1, Screen_Width, 1);
    ...
    if (_sendAnnexDataArray.count) {
        ...
        [HttpTool multipartPostFileDataWithPath:[NSString stringWithFormat:@"%@/annex/fileUpload",urlPrefix] params:params dataAry:self.sendAnnexDataArray success:^(id JSON){
            if ([JSON[@"status"] integerValue] == 200) {
                ...
                selectContactViewController.type = CXTransactionCommitType;
                selectContactViewController.AllAnnexDataArray = weakSelf.annexView.AllAnnexDataArray;
                
                NSMutableDictionary * params = [NSMutableDictionary dictionary];
                [params setValue:weakSelf.titleContentLabel.text forKey:@"name"];
...
- (void)workCreateDetailViewReloadHeightWithThirdViewHeight:(CGFloat)viewHeight
{
    ...
    self.annexLabel.frame = CGRectMake(kLeftSpace, kCellHeight + 1 + kCellHeight + self.contentDetailViewHeight + (kCellHeight - kFontSizeValueForForm)/2, self.annexLabel.size.width, kFontSizeValueForForm);
    self.annexView.frame = CGRectMake(CGRectGetMaxX(self.annexLabel.frame)+ kLeftSpace, kCellHeight + 1 + kCellHeight + self.contentDetailViewHeight, Screen_Width, kCellHeight);
    self.thirdLineView.frame = CGRectMake(0, kCellHeight + 1 + kCellHeight + self.contentDetailViewHeight + kCellHeight + 1, Screen_Width, 1);
    self.thirdLineView.backgroundColor = RGBACOLOR(139, 144, 136, 1.0);
    _theTableHeaderView.frame = CGRectMake(0, 0, Screen_Width, 3*kCellHeight + self.contentDetailViewHeight + 3);;

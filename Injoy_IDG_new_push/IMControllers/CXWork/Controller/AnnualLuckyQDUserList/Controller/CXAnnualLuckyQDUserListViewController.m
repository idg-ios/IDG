//
//  CXAnnualLuckyQDUserListViewController.m
//  InjoyIDG
//
...
@interface CXAnnualLuckyQDUserListViewController ()<UITableViewDataSource, UITableViewDelegate, UISearchDisplayDelegate>
...
@implementation CXAnnualLuckyQDUserListViewController

- (void)viewWillAppear:(BOOL)animated
{
    [super viewWillAppear:animated];
    NSMutableArray* groupUserIDArray = [[NSMutableArray alloc] initWithCapacity:0];
    for (SDCompanyUserModel* member in self.users) {
...
- (void)viewDidLoad
{
    ...
    [self.rootTopView setNavTitle:LocalString(@"签到人员")];
...
//            }];
//        }else{
//            SDContactsDetailController* pivc = [[SDContactsDetailController alloc] init];
//            pivc.userModel = [CXIMHelper getUserByIMAccount:userModel.imAccount];
//            [self.navigationController pushViewController:pivc animated:YES];
//            if ([self.navigationController respondsToSelector:@selector(interactivePopGestureRecognizer)]) {
//                self.navigationController.interactivePopGestureRecognizer.delegate = nil;
//            }

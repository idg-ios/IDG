//
//  CXBorrowingSubmissionViewController.m
//  InjoyERP
//
...
@interface CXBorrowingSubmissionViewController ()<UITableViewDelegate, UITableViewDataSource,CXWorkCreateDetailViewReloadHeightDelegate>
...
@implementation CXBorrowingSubmissionViewController
...
- (void)setupView
{
    ...
    [self.rootTopView setNavTitle:LocalString(@"借支申请")];
    ...
    loanAmountLabel.text = [NSString stringWithFormat:@"借支金额："];
    ...
    if (_sendAnnexDataArray.count) {
        ...
        [HttpTool multipartPostFileDataWithPath:[NSString stringWithFormat:@"%@/annex/fileUpload",urlPrefix] params:params dataAry:self.sendAnnexDataArray success:^(id JSON){
            if ([JSON[@"status"] integerValue] == 200) {
                ...
                selectContactViewController.type = CXBorrowingSubmissionType;

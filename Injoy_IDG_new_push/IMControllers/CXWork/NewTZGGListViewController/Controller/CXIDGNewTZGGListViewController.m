//
//  CXIDGNewTZGGListViewController.m
//  InjoyIDG
//
...
@interface CXIDGNewTZGGListViewController ()<UITableViewDelegate, UITableViewDataSource>

@property(strong, nonatomic) UITableView *listTableView;
/** 数据源 */
@property(strong, nonatomic) NSMutableArray<CXIDGNewTZGGListModel *> *dataSourceArr;
/** 页码 */
@property(nonatomic, assign) NSInteger pageNumber;

@end

@implementation CXIDGNewTZGGListViewController

#pragma mark - get & set

- (NSMutableArray<CXIDGNewTZGGListModel *> *)dataSourceArr {
...
- (void)sortDataSourceArr
{
    if(self.dataSourceArr && [self.dataSourceArr count] > 0){
        NSComparator cmptr = ^(CXIDGNewTZGGListModel * model1, CXIDGNewTZGGListModel * model2){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CXIDGNewTZGGDetailViewController *vc = [[CXIDGNewTZGGDetailViewController alloc] init];
    CXIDGNewTZGGListModel *listModel = self.dataSourceArr[indexPath.section];
    vc.model = listModel;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellID = @"CXIDGTZGGYHListTableViewCell";
    CXIDGTZGGYHListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (nil == cell) {
        cell = [[CXIDGTZGGYHListTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    }
    CXIDGNewTZGGListModel *model = self.dataSourceArr[indexPath.section];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    CXIDGNewTZGGListModel *model = self.dataSourceArr[indexPath.section];
    return [CXIDGTZGGYHListTableViewCell getCXIDGNewTZGGListTableViewCellHeightWithCXIDGNewTZGGListModel:model];
}

#pragma mark <---请求列表--->
- (void)getListWithPage:(NSInteger)page {
    NSString *url = [NSString stringWithFormat:@"%@comNotice/list/%ld", urlPrefix, (long) page];
...
//        [MBProgressHUD hideHUDInMainQueueForView:self.view];//隐藏loading
        ...
        if ([JSON[@"status"] intValue] == 200) {
            ...
            NSArray<CXIDGNewTZGGListModel *> *data = [NSArray yy_modelArrayWithClass:[CXIDGNewTZGGListModel class] json:JSON[@"data"]];
            if (page == 1) {

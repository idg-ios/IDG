//
//  SDIMAllGroupChatMembersViewController.m
...
@interface SDIMAllGroupChatMembersViewController () <UITableViewDataSource, UITableViewDelegate, UISearchDisplayDelegate>
...
@implementation SDIMAllGroupChatMembersViewController

- (void)viewWillAppear:(BOOL)animated
{
    [super viewWillAppear:animated];
    if(self.isSendCall){
        NSMutableArray * users = [[SDDataBaseHelper shareDB] getIMUserData];
        NSMutableArray* groupUserIDArray = [[NSMutableArray alloc] initWithCapacity:0];
        for (SDCompanyUserModel* member in users) {
            [groupUserIDArray addObject:member.hxAccount];
        }
...
- (void)viewDidLoad
{
    ...
    if(self.isSendCall){
        [self.rootTopView setNavTitle:LocalString(@"发起通话")];
    }else{
        [self.rootTopView setNavTitle:LocalString(@"聊天成员")];
    }
    ...
    if (self.groupType != CXGroupTypeVoiceConference && !self.isSendCall && !self.isSystemGroup) {
        // 语音会议不显示按钮
        [self.rootTopView setUpRightBarItemTitle:@"添加" addTarget:self action:@selector(addGroupMembersBtnClick)];
    }
...
- (void)fetchGroupDetailInfo
{
    ...
    [[CXIMService sharedInstance].groupManager getGroupDetailInfoWithGroupId:self.groupId completion:^(CXGroupInfo *group, NSError *error) {
        ...
        if (!error) {
            ...
            NSMutableArray* groupUserIDArray = [[NSMutableArray alloc] initWithCapacity:0];
            for (CXGroupMember* member in wself.group.members) {
                [groupUserIDArray addObject:member.userId];
            }
            wself.members = [NSMutableArray arrayWithArray:groupUserIDArray];
            BOOL ownerIn = NO;
            for (NSString * userID in wself.members) {
                if ([userID isEqualToString:wself.group.owner]) {
                    ownerIn = YES;
                    break;
                }
            }
            if(!ownerIn){
                [wself.members insertObject:self.group.owner atIndex:0];
            }
...
- (void)addGroupMembersBtnClick
{
    ...
    selectColleaguesViewController.filterUsersArray = [NSMutableArray arrayWithArray:[[CXLoaclDataManager sharedInstance]getUsersFromLocalGroupsWithGroupId:self.groupId AndMemberIMAccounts:self.members]];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    [cell setMember:member AndGroupId:self.groupId];
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    __block SDCompanyUserModel* userModel = [[CXLoaclDataManager sharedInstance]getUserByGroupId:self.groupId AndIMAccount:member];
    if(self.isSendCall){
        if (![SDPermissionsDetectionUtils checkCanRecordFree]) {
            ...
            videoCallController.chatter = userModel.hxAccount;
            videoCallController.chatterDisplayName = userModel.realName;

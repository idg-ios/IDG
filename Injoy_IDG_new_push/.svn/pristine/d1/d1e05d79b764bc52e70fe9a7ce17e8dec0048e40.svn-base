...
- (void)setupAssets
{
    ...
    ALAssetsGroupEnumerationResultsBlock resultsBlock = ^(ALAsset *asset, NSUInteger index, BOOL *stop) {
        ...
        {
            //添加之前选的相册对像
            for (NSInteger i = 0; i < self.selectedAssetsArray.count; i ++) {
                
                ALAsset *seletedAsset = self.selectedAssetsArray[i];
                for (ALAsset *alasset in self.assets) {
                    NSString *seletedAssetString = seletedAsset.defaultRepresentation.url.absoluteString;
                    NSString *alassetString = alasset.defaultRepresentation.url.absoluteString;
                    
                    if ([seletedAssetString isEqualToString:alassetString]) {
                        [_indexPathsForSelectedItems addObject:alasset];
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    ...
    if (_indexPathsForSelectedItems.count >= 9) {
        if (self.isNeedRemind) {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"照片数最多只能选择9张,请删除后再添加" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertView show];
        }
        return NO;
    }
...
- (void)setupButtons
{
    ...
    if ([picker.delegate respondsToSelector:@selector(assetPickerControllerWithSelectedAssetArray)]) {
        self.selectedAssetsArray = [picker.delegate assetPickerControllerWithSelectedAssetArray];
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    vc.selectedAssetsArray = self.selectedAssetsArray;
...
@property (nonatomic, weak) ZYQAssetGroupViewController *groupViewController;

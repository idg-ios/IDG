//
//  CXIDGVoiceMeetingSuperCellTableViewCell.m
//  InjoyIDG
//
...
#import "CXIDGVoiceMeetingSuperCellTableViewCell.h"
...
@interface CXIDGVoiceMeetingSuperCellTableViewCell()<UIAlertViewDelegate>

@property (nonatomic,strong) UIMenuController *menuController;

@end

@implementation CXIDGVoiceMeetingSuperCellTableViewCell
...
+(NSString *)identifierForContentType:(CXIMMessageContentType)type{
    ...
    switch (type) {
        ...
        {
            identifier = NSStringFromClass(CXIDGVoiceMeetingCellTableViewCell.class);
            break;
        }
...
- (void)contentViewLongPressed:(UILongPressGestureRecognizer *)gesture{
    ...
    if ([self menuItemEnabled:CXIDGVoiceMeetingSuperCellTableViewCellMenuItemCopy]) {
        [menuItems addObject:[[UIMenuItem alloc] initWithTitle:@"复制" action:@selector(copyTapped)]];
    }
    if ([self menuItemEnabled:CXIDGVoiceMeetingSuperCellTableViewCellMenuItemDelete]){
        [menuItems addObject:[[UIMenuItem alloc] initWithTitle:@"删除" action:@selector(delTapped)]];
    }
    if(!self.isBurnAfterReadMessage){
        if ([self menuItemEnabled:CXIDGVoiceMeetingSuperCellTableViewCellMenuItemForward]){
...
- (BOOL)menuItemEnabled:(CXIDGVoiceMeetingSuperCellTableViewCellMenuItem)item{
    if(self.isBurnAfterReadMessage){
        return item == CXIDGVoiceMeetingSuperCellTableViewCellMenuItemDelete;
    }
    return item == CXIDGVoiceMeetingSuperCellTableViewCellMenuItemDelete || item == CXIDGVoiceMeetingSuperCellTableViewCellMenuItemForward;
}
...
- (void)copyTapped{
    if ([self.delegate respondsToSelector:@selector(chattingCell:didTapMenuItem:message:)]) {
        [self.delegate chattingCell:self didTapMenuItem:CXIDGVoiceMeetingSuperCellTableViewCellMenuItemCopy message:self.message];
    }
}

- (void)delTapped{
    if ([self.delegate respondsToSelector:@selector(chattingCell:didTapMenuItem:message:)]) {
        [self.delegate chattingCell:self didTapMenuItem:CXIDGVoiceMeetingSuperCellTableViewCellMenuItemDelete message:self.message];
    }
}

- (void)forwardTapped{
    if ([self.delegate respondsToSelector:@selector(chattingCell:didTapMenuItem:message:)]) {
        [self.delegate chattingCell:self didTapMenuItem:CXIDGVoiceMeetingSuperCellTableViewCellMenuItemForward message:self.message];
    }
}

//
//  CXDepartmentUserListViewController.m
//  InjoyYJ1
//
...
@interface CXDepartmentUserListViewController ()<UITableViewDataSource, UITableViewDelegate, UISearchDisplayDelegate>
...
@property (nonatomic, strong) NSArray<SDCompanyUserModel *> * users;

@end

@implementation CXDepartmentUserListViewController

- (NSArray<SDCompanyUserModel *> *)users{
    if(!_users){
        _users = @[].copy;
    }
    return _users;
...
- (NSMutableArray *)sortedList{
    if(!_sortedList){
        _sortedList = @[].mutableCopy;
    }
    return _sortedList;
}

- (void)loadData{
    self.users = [[CXLoaclDataManager sharedInstance]getUserFromLocalFriendsContactsDicWithDeptId:self.deptId];
}
...
- (void)viewDidLoad
{
    ...
    [self.rootTopView setNavTitle:LocalString(self.deptName)];
    ...
    for(SDCompanyUserModel * user in self.users){
        [self.members addObject:user.imAccount];
    }
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    CXDepartmentUserListCell* cell = [tableView dequeueReusableCellWithIdentifier:cellName];
    if (cell == nil) {
        cell = [[CXDepartmentUserListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellName];
    }
    NSString* member;
    if (tableView != _tableView) {

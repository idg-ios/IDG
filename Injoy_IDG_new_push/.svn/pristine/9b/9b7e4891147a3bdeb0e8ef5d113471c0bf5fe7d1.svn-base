//
//  CXProjectCollaborationChattingCell.m
//  InjoyDDXWBG
//
...
#import "CXProjectCollaborationChattingVoiceCell.h"
...
@interface CXProjectCollaborationChattingCell() <UIAlertViewDelegate>

@property (nonatomic,strong) UIMenuController *menuController;

@end

@implementation CXProjectCollaborationChattingCell
...
+(NSString *)identifierForContentType:(CXIMMessageContentType)type{
    ...
    switch (type) {
        ...
        case CXIMMessageContentTypeText:
            identifier = NSStringFromClass(CXProjectCollaborationChattingTextCell.class);
            break;
            
            // 图片
        case CXIMMessageContentTypeImage:
        {
            identifier = NSStringFromClass(CXProjectCollaborationChattingImageCell.class);
            break;
        }
        ...
        {
            identifier = NSStringFromClass(CXProjectCollaborationChattingVoiceCell.class);
            break;
        }
...
+(NSString *)identifierForCXIDGSmallBusinessAssistantCellType{
    return NSStringFromClass(CXIDGSmallBusinessAssistantCell.class);
}
...
+(CXProjectCollaborationChattingCell *)createCellForIdentifier:(NSString *)identifier{
    ...
    CXProjectCollaborationChattingCell *cell = [[cellClass alloc] init];
    return cell;
}
...
- (BOOL)isNeedDisplayLocationBtn {
    return NO;
    if (self.isChatMessage && !self.isGroupChat && self.message.ext[@"location"] && !self.isFromSelf) {
        return YES;
    }
    return NO;
}
...
//    if (self.isChatMessage && !self.isGroupChat && self.message.status == CXIMMessageStatusSendSuccess && self.isFromSelf && self.message.readAsk != CXIMMessageReadFlagNoFlag && !self.isNotNeedShowReadOrUnRead) {
//        return YES;
//    }
    return NO;
}
...
- (void)contentViewLongPressed:(UILongPressGestureRecognizer *)gesture{
    ...
    if ([self menuItemEnabled:CXProjectCollaborationChattingCellMenuItemCopy]) {
        [menuItems addObject:[[UIMenuItem alloc] initWithTitle:@"复制" action:@selector(copyTapped)]];
    }
//    if ([self menuItemEnabled:CXProjectCollaborationChattingCellMenuItemDelete]){
//        [menuItems addObject:[[UIMenuItem alloc] initWithTitle:@"删除" action:@selector(delTapped)]];
//    }
//    if(!self.isBurnAfterReadMessage){
//        if ([self menuItemEnabled:CXProjectCollaborationChattingCellMenuItemForward]){
...
//    if(self.isBurnAfterReadMessage){
//        return item == CXProjectCollaborationChattingCellMenuItemDelete;
//    }
//    return item == CXProjectCollaborationChattingCellMenuItemDelete || item == CXProjectCollaborationChattingCellMenuItemForward;
    return item == CXProjectCollaborationChattingCellMenuItemCopy;
}
...
- (void)copyTapped{
    if ([self.delegate respondsToSelector:@selector(projectCollaborationChattingCell:didTapMenuItem:message:)]) {
        [self.delegate projectCollaborationChattingCell:self didTapMenuItem:CXProjectCollaborationChattingCellMenuItemCopy message:self.message];
    }
}

- (void)delTapped{
    if ([self.delegate respondsToSelector:@selector(projectCollaborationChattingCell:didTapMenuItem:message:)]) {
        [self.delegate projectCollaborationChattingCell:self didTapMenuItem:CXProjectCollaborationChattingCellMenuItemDelete message:self.message];
    }
}

- (void)forwardTapped{
    if ([self.delegate respondsToSelector:@selector(projectCollaborationChattingCell:didTapMenuItem:message:)]) {
        [self.delegate projectCollaborationChattingCell:self didTapMenuItem:CXProjectCollaborationChattingCellMenuItemForward message:self.message];
    }
}
...
#pragma mark - 初始化cell
-(void)initCell{
    self.contentView.backgroundColor = kColorWithRGB(232,232,232);
}
...
-(void)setMessage:(CXIMMessage *)message{
    ...
    if (![message.body isKindOfClass:[CXIMSystemNotifyMessageBody class]]) {
        ...
        if (avatarImageView) {
            if([message.sender isEqualToString:@"企业小助手"]){
                avatarImageView.image = [UIImage imageNamed:@"QIYEXIAOZHUSHOU"];
                avatarImageView.highlightedImage = [UIImage imageNamed:@"QIYEXIAOZHUSHOU"];
            }else{
...
- (void)avatarImageViewTapped {
    if ([self.delegate respondsToSelector:@selector(projectCollaborationChattingCell:didTapAvatar:)]) {
        [self.delegate projectCollaborationChattingCell:self didTapAvatar:NULL];
    }
}

//启动阅后即焚定时器
- (void)startBurnAfterReadTimer{
    [self.delegate projectCollaborationChattingCell:self didStartBurnMessagesAfterReadTimer:NULL];
}
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        if ([self.delegate respondsToSelector:(@selector(projectCollaborationChattingCell:didTapSendFailedBtn:))]){
            [self.delegate projectCollaborationChattingCell:self didTapSendFailedBtn:NULL];
        }
    }
}
...
- (void)subClassLayoutSubviews {
    ...
    if (self.isNeedDisplayLocationBtn) {
        if ([self isKindOfClass:[CXProjectCollaborationChattingVoiceCell class]]) {

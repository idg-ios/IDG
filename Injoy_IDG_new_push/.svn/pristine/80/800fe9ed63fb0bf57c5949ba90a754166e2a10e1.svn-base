//
//  CXYunJingWorkCircleAllViewController.m
//  InjoyYJ1
//
...
#import "CXYunJingWorkCircleMyselfViewController.h"
...
@interface CXYunJingWorkCircleAllViewController ()<UITableViewDataSource, UITableViewDelegate,DXFaceDelegate,UITextViewDelegate,IBActionSheetDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate,ALiImageReshapeDelegate>
...
@property (nonatomic, strong) NSMutableArray<NSIndexPath *> * showAllIndexPathArray;

@property (nonatomic, strong) NSMutableArray<NSIndexPath *> * notShowAllIndexPathArray;

@end

@implementation CXYunJingWorkCircleAllViewController

- (NSMutableArray<NSIndexPath *> *)showAllIndexPathArray{
    if(!_showAllIndexPathArray){
        _showAllIndexPathArray = @[].mutableCopy;
    }
    return _showAllIndexPathArray;
}

- (NSMutableArray<NSIndexPath *> *)notShowAllIndexPathArray{
    if(!_notShowAllIndexPathArray){
        _notShowAllIndexPathArray = @[].mutableCopy;
    }
    return _notShowAllIndexPathArray;
}
...
- (void)setupView
{
    ...
    [self.rootTopView setUpRightBarItemImage:[UIImage imageNamed:@"publish"] addTarget:self action:@selector(addNewWork)];
...
- (void)addNewWork
{
    CXYunJingAddNewWorkCircleViewController * yunJingAddNewWorkCircleViewController = [[CXYunJingAddNewWorkCircleViewController alloc] init];
    [self.navigationController pushViewController:yunJingAddNewWorkCircleViewController animated:YES];
...
- (void)receiveNewCommentReloadTable
{
    if([self isHaveUnReadComment]){
        ...
        NSArray * comments = [[SDDataBaseHelper shareDB] getWorkCircleCommentPushModelArray];
        CXWorkCircleCommentPushModel * model = (CXWorkCircleCommentPushModel *)comments[0];
        SDCompanyUserModel * comUserModel = [[CXLoaclDataManager sharedInstance]getUserFromLocalFriendsContactsDicWithUserId:[model.userId integerValue]];
        [_commentNotificationHeadImageView sd_setImageWithURL:[NSURL URLWithString:comUserModel.icon?comUserModel.icon:model.icon] placeholderImage:[UIImage imageNamed:@"temp_user_head"] options:EMSDWebImageRetryFailed];
...
{
    CXYunJingWorkCircleMyselfViewController * myselfWorkCircleViewController = [[CXYunJingWorkCircleMyselfViewController alloc] init];
...
- (void)downloadData
{
    ...
    [HttpTool multipartPostFileDataWithPath:url params:nil dataAry:nil success:^(id JSON) {
        ...
        if ([jsonDict[@"status"] integerValue] == 200) {
            [self.showAllIndexPathArray removeAllObjects];
            [self.notShowAllIndexPathArray removeAllObjects];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    CXYunJingWorkCircleTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellName];
    if(!cell){
        cell = [[CXYunJingWorkCircleTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellName];
        CXWeakSelf(self)
        cell.updateCellHeightCallBack = ^(NSIndexPath * indexPath,ShowCellType type){
            if(type == MoreAllShowCellType){
                [weakself.showAllIndexPathArray addObject:indexPath];
                [weakself.notShowAllIndexPathArray removeObject:indexPath];
                [self.tableView reloadData];
                
            }else if(type == MoreNotAllShowCellType){
                [weakself.notShowAllIndexPathArray addObject:indexPath];
                [weakself.showAllIndexPathArray removeObject:indexPath];
                [self.tableView reloadData];
            }
        };
    }else{
        if([self.showAllIndexPathArray containsObject:indexPath]){
            cell.type = MoreAllShowCellType;
        }else if([self.notShowAllIndexPathArray containsObject:indexPath]){
            cell.type = MoreNotAllShowCellType;
        }else{
            cell.type = LessAllShowCellType;
        }
    }
    [cell setCXAllPeoplleWorkCircleModel:self.dataSource[indexPath.row] AndNSIndexPath:indexPath];
...
- (CGFloat)tableView:(UITableView*)tableView heightForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    return [CXYunJingWorkCircleTableViewCell getYujingCellHeightWithModel:self.dataSource[indexPath.row] AndType:type];
}

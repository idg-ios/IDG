...
{
    if ([[url relativeString] rangeOfString:@"http"].location != NSNotFound) {
        [self sd_setImageWithURL:url placeholderImage:placeholder options:options progress:nil completed:nil];
    }
    else {
        NSURL* url_ = url;
        if ([url.relativeString length] > 1) {
            if (YES == [url.relativeString hasPrefix:@"/"]) {
                url_ = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", urlPrefix, [url.relativeString substringFromIndex:1]]];
            }
            else {
                url_ = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", urlPrefix, url.relativeString]];
            }
        }
        [self sd_setImageWithURL:url_ placeholderImage:placeholder options:options progress:nil completed:nil];
    }
}
...
- (void)sd_setImageWithURL:(NSURL*)url placeholderImage:(UIImage*)placeholder options:(EMSDWebImageOptions)options progress:(EMSDWebImageDownloaderProgressBlock)progressBlock completed:(EMSDWebImageCompletionBlock)completedBlock
{
    if ([[url relativeString] rangeOfString:@"http"].location == NSNotFound) {
        if ([url.relativeString length] > 1) {
            url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", urlPrefix, [url.relativeString substringFromIndex:1]]];
        }
    }
    ...
    if (url) {
        ...
        id<EMSDWebImageOperation> operation = [EMSDWebImageManager.sharedManager downloadImageWithURL:url
                                                                                              options:options
...
- (void)sd_setAnimationImagesWithURLs:(NSArray*)arrayOfURLs
{
    ...
    for (NSURL* logoImageURL in arrayOfURLs) {
        id<EMSDWebImageOperation> operation = [EMSDWebImageManager.sharedManager downloadImageWithURL:logoImageURL
                                                                                              options:0
                                                                                             progress:nil
                                                                                            completed:^(UIImage* image, NSError* error, EMSDImageCacheType cacheType, BOOL finished, NSURL* imageURL) {
                                                                                                if (!wself)

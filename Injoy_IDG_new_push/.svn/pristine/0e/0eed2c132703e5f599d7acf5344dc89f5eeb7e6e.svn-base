//
//  CXIDGNewCapitalExpressTableViewCell.m
//  InjoyIDG
//
...
@interface CXIDGNewCapitalExpressTableViewCell()
...
@implementation CXIDGNewCapitalExpressTableViewCell
...
- (void)layoutUI
{
    ...
    if(_model.thumb_url && [_model.thumb_url length] > 0){
        ...
        _readAllLabel.text = !_model.digest||[_model.digest length] <= 0?@" ":_model.digest;
...
+ (CGFloat)getCXIDGCapitalExpressListTableViewCellHeightWithCXIDGCapitalExpressListModel:(CXIDGCapitalExpressListModel *)model
{
    ...
    if(model.thumb_url && [model.thumb_url length] > 0){
        [thumb_urlImageView sd_setImageWithURL:[NSURL URLWithString:(model.thumb_url && ![model.thumb_url isKindOfClass:[NSNull class]])?model.thumb_url:@""] placeholderImage:[UIImage imageNamed:@""] options:EMSDWebImageRetryFailed];
        ...
        readAllLabel.text = !model.digest||[model.digest length] <= 0?@" ":model.digest;

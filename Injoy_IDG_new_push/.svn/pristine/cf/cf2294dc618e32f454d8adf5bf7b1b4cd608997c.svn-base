    ...
    // 设置刷新样式
    {
        NSArray *idleImages = ({
            ...
            for (NSUInteger i = 1; i <= 18; i++) {
                NSString *imgName = [NSString stringWithFormat:@"refresh_anim-%zd", i];
                UIImage *img = [UIImage imageNamed:imgName];
                NSAssert(img != nil, @"图片不存在");
                [arr addObject:img];
            }
            arr.copy;
        });
...
+ (void)load
{
    ...
    /* 替换刷新样式 */
    {
        method1 = class_getInstanceMethod([self class], @selector(addLegendHeader));
        method2 = class_getInstanceMethod([self class], @selector(addGifHeader));
        method_exchangeImplementations(method1, method2);
        
        method1 = class_getInstanceMethod([self class], @selector(legendHeader));
        method2 = class_getInstanceMethod([self class], @selector(gifHeader));
        method_exchangeImplementations(method1, method2);
        
        method1 = class_getInstanceMethod([self class], @selector(addLegendFooter));
        method2 = class_getInstanceMethod([self class], @selector(addGifFooter));
        method_exchangeImplementations(method1, method2);
        
        method1 = class_getInstanceMethod([self class], @selector(legendFooter));
        method2 = class_getInstanceMethod([self class], @selector(gifFooter));
        method_exchangeImplementations(method1, method2);
    }
}

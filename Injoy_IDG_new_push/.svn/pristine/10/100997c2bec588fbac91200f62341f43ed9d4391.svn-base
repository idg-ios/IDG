//
//  CXPaymentDetailViewController.m
//  InjoyIDG
//
...
@interface CXPaymentDetailViewController ()<UITableViewDelegate, UITableViewDataSource, CXAnnexDownloadDelegate>
...
@implementation CXPaymentDetailViewController
...
- (void)setUpNavBar {
    ...
    if ([self.listModel.itemType isEqualToString:@"Payment"]) {
        [rootTopView setNavTitle:@"付款单"];
    } else if([self.listModel.itemType isEqualToString:@"PaymentVoucher"]){
        [rootTopView setNavTitle:@"付款单"];
    }
...
- (void)setUpTableView {
    ...
    _jkdwLabel.title = @"付款单位：";
    _jkdwLabel.textColor = kTextColor;
    _jkdwLabel.allowEditing = NO;
    _jkdwLabel.content = self.model.company ? : self.model.payCompany;
    ...
    _jkdwLabel.didFinishEditingBlock = ^(CXEditLabel *editLabel) {
        CXStrongSelf(self)
        self.model.payCompany = editLabel.content;
    };
...
//        self.model.remark = editLabel.content;
    ...
    line_6.frame = CGRectMake(0.f, _opinionLabel.bottom, lineWidth, lineBoldHeight);
    */
    
    //一级审批
    _yjspLabel = [[CXEditLabel alloc] initWithFrame:CGRectMake(leftMargin, _bzLabel.bottom, viewWidth, viewHeight)];
    ...
    //二级审批
    _ejspLabel = [[CXEditLabel alloc] initWithFrame:CGRectMake(_yjspLabel.right, _bzLabel.bottom, viewWidth, viewHeight)];
    ...
    _hkzpdwxxBackView.frame = CGRectMake(0.f, _cnLabel.bottom, lineWidth, kHKZPDWXXLabelTopSpace + kHKZPDWXXLabelFontSize + kHKZPDWXXLabelBottomSpace);
...
- (void)buttonClick:(UIButton *)sender{
    NSLog(@"付款单,点击的是同意?===%@",sender.titleLabel.text);
...
- (void)showActionController{
    ...
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (remark.length == 0 || [remark isEqualToString:@""]) {
            [MBProgressHUD toastAtCenterForView:self.view text:@"批审意见不能为空" duration:3];
            [self performSelector:@selector(showActionController) withObject:nil afterDelay:2.0];
        }else{
            [self submitWithReason:remark agree:NO];
        }
    }];

...
- (void)setUpNavBar {
    ...
    [rootTopView setUpRightBarItemImage:Image(@"add") addTarget:self action:@selector(rightBarItemOnTap)];
    [rootTopView setUpRightBarItemImage2:Image(@"msgSearch") addTarget:self action:@selector(searchBtnClick)];
}
...
- (void)setUpTableView {
    ...
    [self.listTableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.view).offset(navHigh);
    }];
...
- (void)rightBarItemOnTap{
    ...
    selectColleaguesViewController.filterUsersArray = [NSMutableArray arrayWithArray:@[[[CXLoaclDataManager sharedInstance] getUserFromLocalFriendsDicWithIMAccount:[AppDelegate getUserHXAccount]]]];

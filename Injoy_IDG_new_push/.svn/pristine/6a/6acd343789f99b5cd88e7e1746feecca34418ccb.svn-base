...
+ (instancetype)photoViewWithFrame:(CGRect)frame atImageUrlString:(NSString*)urlString imageName:(NSString*)imageName
{
    return [[self alloc] initWithFrame:frame atImageUrlString:urlString imageName:imageName];
}

- (instancetype)initWithFrame:(CGRect)frame atImageUrlString:(NSString*)urlString imageName:(NSString*)imageName
{
    ...
    if (self) {
        ...
        self.delegate = self;
        [self imageView];
        [self setupImageOfURLString:urlString imageName:imageName];
    }
    return self;
}

#pragma mark - 根据传入的urlString 设置UIImageView的image
- (void)setupImageOfURLString:(NSString*)urlString imageName:(NSString*)imageName
{
    ...
    /** 取得图片名 */
    NSString* imageName_ = [urlString lastPathComponent];

    /** 取得图片扩展名 */
    NSString* imageExtension = [imageName pathExtension];

    /** 先从沙盒中取出图片 */
    __block UIImage* image = [UIImage loadImage:imageName_ ofType:imageExtension inDirectory:cachesDirectory];
...
//                                           weakSelf.imageView.frame = weakSelf.containerView.bounds;
                                           [weakSelf setup];
                                           [weakSelf layoutSubviews];
                                       });
                                   }
                                   /** 下载图片后存储到沙盒中 */
                                   [image saveImage:image atFileName:imageName_ atImageType:imageExtension atDirectory:cachesDirectory];
                               }];
    }
}

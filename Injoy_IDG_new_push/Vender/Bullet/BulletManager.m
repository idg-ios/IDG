...
@property (nonatomic, strong) NSMutableArray<CXIDGNHDMModel *> *allComments;
@property (nonatomic, strong) NSMutableArray<CXIDGNHDMModel *> *tmpComments;
 ...
 */
- (void)createBulletComment:(CXIDGNHDMModel *)comment trajectory:(Trajectory)trajectory {
    ...
    view.moveBlock = ^(CommentMoveStatus status) {
        ...
        switch (status) {
            ...
            case Enter: {
                ...
                CXIDGNHDMModel *comment = [myself nextComment];
                if (comment) {
            ...
            case End: {
                ...
                [self.tmpComments removeObject:comment];
...
- (void)initBulletCommentView {
    //初始化三条弹幕轨迹
    NSMutableArray *arr = [NSMutableArray arrayWithArray:@[@(0), @(1), @(2), @(3), @(4), @(5), @(6), @(7)]];
    for (int i = 8; i > 0; i--) {
        CXIDGNHDMModel *comment = [self.tmpComments firstObject];
        if (comment) {
...
- (void)addComments:(NSArray<CXIDGNHDMModel *> *)comments{
    [self.tmpComments addObjectsFromArray:comments];
}

#pragma mark - Getter

- (NSMutableArray<CXIDGNHDMModel *> *)allComments {
    if (!_allComments) {
...
- (NSMutableArray<CXIDGNHDMModel *> *)tmpComments {
    if (!_tmpComments) {

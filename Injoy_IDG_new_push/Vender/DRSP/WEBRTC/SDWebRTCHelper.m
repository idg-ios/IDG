...
@interface SDWebRTCHelper () <SRWebSocketDelegate, RTCPeerConnectionDelegate>
{
...
@implementation SDWebRTCHelper
#pragma mark - singleton
static SDWebRTCHelper *shareInstance;
...
- (void)_setupForLocalStream
{
    ...
    if (!self.localStream) {
        ...
        self.audioTrack = [_factory audioTrackWithTrackId:@"ARDAMSa0"];
        AVAuthorizationStatus authStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
        NSArray<AVCaptureDevice *> *devices;
        
        self.localStream = [_factory mediaStreamWithStreamId:@"ARDAMS"];
        // 添加音频轨迹
        [self.localStream addAudioTrack:self.audioTrack];
        
        if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) { // 摄像头权限
            BKLog(@"相机访问受限");
            if ([_delegate respondsToSelector:@selector(webRTCHelper:setLocalStream:connectionID:)]) {
                [_delegate webRTCHelper:self setLocalStream:nil connectionID:_myId];
            }
        }
        else {
            ...
            if (device) {
                videoSource = [_factory avFoundationVideoSourceWithConstraints:[self _setupForLocalVideoConstraints]];
                [videoSource setUseBackCamera:NO];
                self.videoTrack = [_factory videoTrackWithSource:videoSource trackId:@"ARDAMSv0"];
                
                // 添加视频轨迹
                [self.localStream addVideoTrack:self.videoTrack];
                
                if ([_delegate respondsToSelector:@selector(webRTCHelper:setLocalStream:connectionID:)]) {
                    [_delegate webRTCHelper:self setLocalStream:self.localStream connectionID:_myId];
                }
            }
            else {
                BKLog(@"该设备不能打开摄像头");
...
-(void)camareChangeWithIsBackCamare:(BOOL)isBackCamare{
    [videoSource setUseBackCamera:isBackCamare];
}
...
{
    BKLog(@"");
    NSDictionary *mandatory = @{kRTCMediaConstraintsMinFrameRate : @"30",
                                kRTCMediaConstraintsMaxFrameRate : @"70"
                                };
...
- (void)_setupForIceServers
{
    ...
    if (!_iceServers) {
        ...
        [_iceServers addObject:[self _setupForIceServer:RTCSTUNServerURL Username:@"" Credential:@""]];
        [_iceServers addObject:[self _setupForIceServer:RTCSTUNServerURL2 Username:@"" Credential:@""]];
        [_iceServers addObject:[self _setupForIceServer:kICEServer_URL Username:kICEServer_UserName Credential:kICEServer_Password]];
    }
}

- (RTCIceServer *)_setupForIceServer:(NSString *)stunURL Username:(NSString *)username Credential:(NSString *)credential
{
    BKLog(@"");
    return [[RTCIceServer alloc] initWithURLStrings:@[stunURL] username:username credential:credential];
}

...
- (void)viewDidLoad
{
    ...
    [self.rootTopView setNavTitle:@"选择人员"];
    ...
    self.tableView.frame = CGRectMake(0, CGRectGetMaxY(self.searchBar.frame), self.view.frame.size.width, self.view.frame.size.height - self.searchBar.frame.size.height - self.footerView.frame.size.height - CGRectGetHeight(self.rootTopView.frame));
...
{
    if (_searchBar == nil) {
        _searchBar = [[EMSearchBar alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.rootTopView.frame), self.view.frame.size.width, 44)];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    cell.imageView.image = [UIImage imageNamed:@"chatListCellHead.png"];
    cell.textLabel.text = [[[SDChatManager sharedChatManager] searchUserByHxAccount:buddy.username] realName];
...
- (void)reloadFooterView
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        remarkView.remark = [[[SDChatManager sharedChatManager] searchUserByHxAccount:buddy.username] realName];
...
- (void)loadDataSource
{
    ...
    NSArray*buddyList = [[SDChatManager sharedChatManager] embuddyAry];
